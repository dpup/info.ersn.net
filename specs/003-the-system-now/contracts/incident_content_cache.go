// Package contracts defines the interface contracts for incident content deduplication
// Focuses on background processing to achieve <200ms API response times
//
// CONTRACT TESTS: These interfaces MUST have failing tests before implementation  
// TDD REQUIREMENT: All tests in this file must fail until implementation exists
...
// ProcessedIncidentCache stores background-processed incident data
type ProcessedIncidentCache struct {
	// ContentHash uniquely identifies this incident across feed refreshes
	ContentHash       IncidentContentHash `json:"content_hash"`
	// Stage indicates what level of processing has been applied
	Stage            ProcessingStage     `json:"stage"`
	// OriginalIncident is the raw Caltrans data
	OriginalIncident interface{}         `json:"original_incident"`
	// ProcessedData contains the enhanced/classified result
	ProcessedData    interface{}         `json:"processed_data"`
	// LastSeenInFeed tracks when incident last appeared in Caltrans feeds
	LastSeenInFeed   time.Time          `json:"last_seen_in_feed"`
	// CacheExpiresAt implements 1-hour expiration after disappearing from feeds
	CacheExpiresAt   time.Time          `json:"cache_expires_at"`
	// ServeCount tracks how many times this cached result was served
	ServeCount       int64              `json:"serve_count"`
	// ProcessingDuration tracks how long background processing took
...
type StoreConfig struct {
	// ProcessingIntervalMinutes how often to check for new incidents
	ProcessingIntervalMinutes  int  `json:"processing_interval_minutes" koanf:"processing_interval_minutes"`
	// MaxConcurrentOpenAI limit concurrent OpenAI API calls
	MaxConcurrentOpenAI        int  `json:"max_concurrent_openai" koanf:"max_concurrent_openai"`
	// CacheTTLHours how long to keep processed incidents (after last seen)
	CacheTTLHours             int  `json:"cache_ttl_hours" koanf:"cache_ttl_hours"`
	// PrefetchEnabled whether to proactively process common incidents
	PrefetchEnabled           bool `json:"prefetch_enabled" koanf:"prefetch_enabled"`
	// OpenAITimeoutSeconds timeout for individual OpenAI calls
	OpenAITimeoutSeconds      int  `json:"openai_timeout_seconds" koanf:"openai_timeout_seconds"`
}

// GetDefaultStoreConfig provides recommended configuration values
